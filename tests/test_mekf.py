...
class Test_AttMEKF:
    ...
    @fixture
    def mekf(self):
        return ap.AttMEKF(10.0, (1.0, 0.0, 0.0, 0.0))
    ...
    def test__init__default(self):
        fs = 10.0
        mekf = ap.AttMEKF(fs, (1.0, 0.0, 0.0, 0.0))
    ...
    def test__init__nav_frame(self):
        ...
        mekf_ned = ap.AttMEKF(10.0, att, nav_frame="NED")
        np.testing.assert_allclose(mekf_ned._z2g, 1.0)

        mekf_enu = ap.AttMEKF(10.0, att, nav_frame="ENU")
        np.testing.assert_allclose(mekf_enu._z2g, -1.0)

        with pytest.raises(ValueError):
            ap.AttMEKF(10.0, att, nav_frame="invalid")
    ...
    def test_bias_gyro(self, att):
        mekf = ap.AttMEKF(10.0, att, bg=np.array([0.01, -0.02, 0.03]))
        bg_expected = np.array([0.01, -0.02, 0.03])
        np.testing.assert_allclose(mekf.bias_gyro, bg_expected)
        assert mekf.bias_gyro is not mekf._bg_b  # ensure it is a copy
    ...
    def test_angular_rate(self, att):
        w = np.array([0.1, -0.2, 0.3])
        mekf = ap.AttMEKF(10.0, att, w=w)
        np.testing.assert_allclose(mekf.angular_rate, w)
        assert mekf.angular_rate is not mekf._w_b  # ensure it is a copy

    def test_P(self, mekf, att):
        mekf = ap.AttMEKF(10.0, att, P=np.eye(6))
