...
class FixedIntervalSmoother:
    ...
    def __init__(self, ains: AHRS, cov_smoothing: bool = True) -> None:
        ...
        self._x = np.empty((0, 7), dtype="float64")
    ...
    def quaternion(self) -> NDArray:
        ...
        np.ndarray, shape (N, 4)
            Unit quaternion estimates for each of the N time steps where the smoother
    ...
    def bias_gyro(self, degrees: bool = False) -> NDArray:
        ...
        np.ndarray, shape (N, 3)
            Gyroscope bias estimates for each of the N time steps where the smoother
            has been updated with measurements.
        """
        x = self.x
        if x.size == 0:
...
def _rts_backward_sweep(
    ...
    Parameters
    ----------
    x : NDArray, shape (n_samples, 7)
        The state vector.
    dx : NDArray, shape (n_samples, 6)
        The error state vector.
    P : NDArray, shape (n_samples, 6, 6)
        The covariance matrix.
    P_prior : NDArray, shape (n_samples, 6, 6)
        The a priori covariance matrix.
    phi : NDArray, shape (n_samples, 6, 6)
    ...
    Returns
    -------
    x_smth : NDArray, shape (n_samples, 7)
        The smoothed state vector.
    P_smth : NDArray, shape (n_samples, 6, 6)
    ...
    for k in range(len(x) - 2, -1, -1):
        ...
        x[k][0:4] = _quaternion_product(x[k][0:4], ddq)
        x[k][0:4] = _normalize(x[k][0:4])
        x[k][4:7] = x[k][4:7] + ddx[3:6]
